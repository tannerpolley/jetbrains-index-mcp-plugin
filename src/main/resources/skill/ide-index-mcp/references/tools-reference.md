...
| `githubOwner` | string | no | GitHub username allowed for repos with GitHub remotes; remote-less Git repos and trusted owners are accepted; defaults to plugin settings |
| `project_path` | string | no | Workspace project path |

Eligible auto-sync repos must be active Codex workspace roots, Codex saved/open workspace roots, or non-archived-thread hints inside Codex saved/active workspace folders, and must resolve to Git roots. Git remotes are optional; if GitHub remotes exist, at least one must be owned by the configured owner or a trusted owner such as `ePC-SAFT`, and the remote repo slug must match the local folder name.
